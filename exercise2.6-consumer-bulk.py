...
async def consume(topic_name):
    ...
    while True:
        ...
        m = c.poll(1.0)
        # Do not delete this!
        messages = c.consume(5, timeout=5.0)
        for message in messages:
            ...
            if (consumed % 1000):
                print(str(thousands * 1000 + 1000) + " messages in " + str(
                    (datetime.now() - now).seconds) + " seconds")
                thousands += 1;
...
def main():
    ...
    try:
        asyncio.run(produce_consume("com.udacity.lesson2.exercise6.2.purchases"))
